...
@section('content')
    ...
    <form class="flex-col space-y-10" action="{{ isset($post) ? route('posts.update', $post->id) : route('posts.store') }}" method="POST">
        ...
        <div>
            ...
            <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="Title" id="content"  name="content">{{ old('content', isset($post) ? $post->content : '') }}</textarea>
        </div>
